...
def get_product_links(sheet, driver, workbook):
    with open('plst_entire_urls_women_outerwear.txt', 'r', encoding='utf-8') as file:
    ...
    for url_product in urls:
        ...
        if "PLST(プラステ)公式 |" in page_title:
            ...
            workbook.save('Product Information PLST women.xlsx')
        else:
            driver.refresh()
            time.sleep(3)
            product_info = scraper(driver, url_product)
...
def main():
    with setup_driver() as driver:
        try:
            ...
            file_name = "Product Information PLST women.xlsx"
